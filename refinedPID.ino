...
void setup()                            // Inicializamos todo las variables que sean necesarias, configuramos los pines de entrada/salida y el terminal serie.
{
  ...
  SampleTime = 25;                      // Se le asigna el tiempo de muestreo en milisegundos.
  ...
  if (Serial.available() > 0)           // Comprueba si ha recibido algún dato por el terminal serie.
  {
    ...
    if (cmd > 31)
    {
      ...
      {                                                                                // También se puede poner individualmente, por ejemplo "p5.5", sólo cambiará el parámetro kp, los mismo si son de dos en dos.
        case 'P': kp  = Serial.parseFloat(); SetTunings(kp, ki, kd); flags = 1; break; // Carga las constantes y presenta en el terminal serie los valores de las variables que hayan sido modificadas.
        case 'I': ki  = Serial.parseFloat(); SetTunings(kp, ki, kd); flags = 1; break;
        case 'D': kd  = Serial.parseFloat(); SetTunings(kp, ki, kd); flags = 1; break;
...
void SetTunings(double kp, double ki, double kd)  // A las constantes KI y KD se les incluyen el tiempo de muestreo. De esta manera se realiza el cálculo una sola vez.
{
