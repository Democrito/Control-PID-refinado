...
void setup()                          // Inicializamos todo las variables que sean necesarias, configuramos los pines de entrada/salida y configuramos el terminal serie.
{
  ...
  imprimir(false);                    // Muestra los datos de sintonización y el tiempo de muestreo por el terminal serie.
}
...
void loop()
{
  ...
  // *********************************************** Control del Motor *************************************************
  if (abs(error) == 0.0)                // Cuando está muy cerca del punto designado, parar el motor.
  { 
...
double Compute(void)
{
   ...
   if(timeChange >= SampleTime)                   // Si se cumple el tiempo de muestreo entonces calcula la salida.
   {
     ...
     if ((dInput == 0.0) || (abs(error) == 0.0)) ITerm += (error * ki); else ITerm -= (dInput / (kp * ki * kd)); // Evita sumar el error integral al proporcional cuando no le corresponde y suaviza la llegada al punto designado.
