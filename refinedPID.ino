...
double Compute(void)
{
   ...
   if(timeChange >= SampleTime)                   // Si se cumple el tiempo de muestreo entonces calcula la salida.
   {
     ...
     // Para una mayor suavidad al acercarse al punto designado puedes cambiar el "else" de arriba por este otro: else ITerm -= (dInput / (kn * ki * kd));
...
void setup()                          // Inicializamos todo las variables que sean necesarias, configuramos los pines de entrada/salida y configuramos el terminal serie.
{
  ...
  window = kp;                        // Ventana de error permitido. Uso KP y me da buen resultado, pero si tienes problemas pon una cifra decimal (ejemplo: 3.2) que se adapte a tus necesidades.
...
void loop()
{
  ...
  if (Serial.available() > 0)        // Si recibe algo por el terminal serie...
  {
    ...
    {                                                   // También se puede poner individualmente, por ejemplo "p5.5", sólo cambiará el parámetro kp, los mismo si son de dos en dos.
      ...
      switch(cmd) 
      {
        case 'P': kp=Serial.parseFloat(); SetTunings(kp, ki, kd); imprimir(); break; // Carga las constantes y presenta en el terminal serie los valores de las variables que hayan sido modificadas.
